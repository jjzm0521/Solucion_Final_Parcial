...
class DiffractionSimulator:
    ...
    def create_aperture(self):
        ...
        if self.current_scene == 'Círculo':
            ...
            return (np.abs(X) <= self.rect_width/2) & (np.abs(Y) <= self.rect_height/2)
    ...
    def create_gato(self):
        ...
        oreja_izq = np.zeros((self.N, self.N), dtype=bool)
        oreja_der = np.zeros((self.N, self.N), dtype=bool)
        
        oreja_izq_y_start = self.N//2 - self.N//4 - self.N//8
        oreja_izq_y_end = self.N//2 - self.N//4
        oreja_izq_x_start = self.N//4
        oreja_izq_x_end = self.N//4 + self.N//8
        
        oreja_der_y_start = self.N//2 - self.N//4 - self.N//8
        oreja_der_y_end = self.N//2 - self.N//4
        oreja_der_x_start = 3*self.N//4 - self.N//8
        oreja_der_x_end = 3*self.N//4

        # Ajustar para que quepa en la máscara
        oreja_izq_y_slice = slice(oreja_izq_y_start, oreja_izq_y_end)
        oreja_izq_x_slice = slice(oreja_izq_x_start, oreja_izq_x_end)
        
        oreja_der_y_slice = slice(oreja_der_y_start, oreja_der_y_end)
        oreja_der_x_slice = slice(oreja_der_x_start, oreja_der_x_end)

        mask[oreja_izq_y_slice, oreja_izq_x_slice] = triangulo[:oreja_izq_y_end-oreja_izq_y_start, :oreja_izq_x_end-oreja_izq_x_start]
        mask[oreja_der_y_slice, oreja_der_x_slice] = triangulo[:oreja_der_y_end-oreja_der_y_start, :oreja_der_x_end-oreja_der_x_start]
    ...
    def setup_ui(self):
        ...
        self.sliders = {
            'Círculo': [
                Slider(plt.axes([0.3, 0.15, 0.6, 0.03], facecolor=self.axcolor), 'Diametro (mm)', 0.1, 5, valinit=self.circle_diameter * 1e3)
            ],
            'Rectángulo': [
                Slider(plt.axes([0.3, 0.15, 0.6, 0.03], facecolor=self.axcolor), 'Ancho (mm)', 0.1, 5, valinit=self.rect_width * 1e3),
                Slider(plt.axes([0.3, 0.1, 0.6, 0.03], facecolor=self.axcolor), 'Alto (mm)', 0.1, 5, valinit=self.rect_height * 1e3)
            ],
            'Doble Rendija': [
                Slider(plt.axes([0.3, 0.15, 0.6, 0.03], facecolor=self.axcolor), 'Ancho (mm)', 0.01, 0.5, valinit=self.slit_width * 1e3),
                Slider(plt.axes([0.3, 0.1, 0.6, 0.03], facecolor=self.axcolor), 'Separacion (mm)', 0.1, 2, valinit=self.slit_separation * 1e3)
            ],
            'Matriz de Puntos 4x4': [
                Slider(plt.axes([0.3, 0.15, 0.6, 0.03], facecolor=self.axcolor), 'Tamaño Cuadrícula (mm)', 0.1, 3, valinit=self.grid_size * 1e3),
                Slider(plt.axes([0.3, 0.1, 0.6, 0.03], facecolor=self.axcolor), 'Radio Punto (mm)', 0.01, 0.5, valinit=self.punto_radio * 1e3)
            ],
            'Flor': [
                Slider(plt.axes([0.3, 0.15, 0.6, 0.03], facecolor=self.axcolor), 'Pétalos', 3, 20, valinit=self.flor_petalos, valstep=1),
    ...
    def update_plot(self, event=None):
        ...
        if not hasattr(self, 'im1'):
            self.im1 = self.ax1.imshow(aperture, cmap='gray', extent=[-self.N/2*self.dx*1e3, self.N/2*self.dx*1e3, -self.N/2*self.dx*1e3, self.N/2*self.dx*1e3])
            self.ax1.set_title('Abertura')
            self.ax1.set_xlabel('x (mm)')
            self.ax1.set_ylabel('y (mm)')
        else:
            self.im1.set_data(aperture)
        ...
        # Ensure indices are valid before creating a view
        if np.any(idx_x) and np.any(idx_y):
            I_zoom = intensity[np.outer(idx_y, idx_x)].reshape((np.sum(idx_y), np.sum(idx_x)))
        else:
            I_zoom = np.zeros((1,1)) # Fallback to a single pixel if no data is in zoom range

        interpolation_method = 'bicubic' if not self.is_dragging else 'none'

        if not hasattr(self, 'im2'):
            self.im2 = self.ax2.imshow(I_zoom, cmap='inferno', norm=PowerNorm(gamma=0.3), extent=extent_difraccion, interpolation=interpolation_method)
            self.ax2.set_title('Patrón de Difracción')
            self.ax2.set_xlabel("x' (mm)")
            self.ax2.set_ylabel("y' (mm)")
        else:
            self.im2.set_data(I_zoom)
            self.im2.set_extent(extent_difraccion)
            self.im2.set_interpolation(interpolation_method)

        self.ax2.set_xlim(-self.zoom, self.zoom)
        self.ax2.set_ylim(-self.zoom, self.zoom)

        perfil = I_zoom[I_zoom.shape[0] // 2, :]
        x_perfil = np.linspace(-zoom_m*1e3, zoom_m*1e3, len(perfil))
        
        if not hasattr(self, 'line3'):
            self.line3, = self.ax3.plot(x_perfil, perfil)
            self.ax3.set_title('Perfil de Intensidad Central')
            self.ax3.set_xlabel("x' (mm)")
            self.ax3.set_ylabel('Intensidad')
            self.ax3.grid(True)
        else:
            self.line3.set_data(x_perfil, perfil)
            self.ax3.relim()
